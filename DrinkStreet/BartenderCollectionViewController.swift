...
class BartenderCollectionViewController: UICollectionViewController {
    ...
    */
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "BartenderCollectionViewCell", for: indexPath) as! BartenderCollectionViewCell
    
        // Configure the cell
        let barDetails = drinkDetail[indexPath.row]
        let url = URL(string: "\(barDetails.strDrinkThumb)")
        if let dataImage = try? Data(contentsOf: url!){
            cell.barImageView.image = UIImage(data: dataImage)
        }
        cell.barCategory.text = barDetails.strCategory
